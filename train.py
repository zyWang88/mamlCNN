...
def main():
    ...
    if args.filename == None:
        file_name = '{}way{}shot-{}-{}'.format(args.n_way,args.k_spt,args.embedding,args.type)
        dt = datetime.now()
    ...
    if args.embedding == 'glove':
        ...
        val_data_loader = glove_getloader(args.val, tokenizer, N=args.n_way, K=args.k_spt, Q=args.k_qry,
        ...
        train_data_loader = bert_getloader(args.train, tokenizer, N=args.n_way, K=args.k_spt, Q=args.k_qry,
                                           na_rate=args.na_rate, batch_size=args.task_num)
        val_data_loader = bert_getloader(args.val, tokenizer, N=args.n_way, K=args.k_spt, Q=args.k_qry,
                                         batch_size=args.task_num)
    ...
    for step in range(args.epoch):
        ...
        if step % 10 == 0:
            ...
            with open(file_name,'a') as f:
                f.write("\nstep: {}\ttraining acc:{}\tloss:{}\tcost:{}min".format(step,accs,loss,(time.time() - start) // 60))
        if step % 30 == 0:
            accs = []
            ...
            with open(file_name,'a') as f:
                f.write("\nTest acc:{}\tcost:{}min".format(accs,(time.time() - start) // 60))
            print('Test acc:', accs, '\tcost', (time.time() - start) // 60, 'min\n')
            if best_result <= accs[-1]:
